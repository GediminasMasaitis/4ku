...
const i32 max_material[] = {125, 406, 441, 767, 1468, 0, 0};
const i32 material[] = {S(106, 125), S(378, 406), S(391, 441), S(492, 767), S(975, 1468), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(-1, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-6, -5), S(-4, -2), S(-1, 0), S(1, 3), S(5, 4), S(10, 1), S(6, -1), S(-11, -1)},
    {S(-5, -3), S(-1, -1), 0, S(2, 2), S(3, 2), S(6, 0), S(2, 0), S(-8, 0)},
    {S(-2, -2), S(-4, -3), S(-5, -2), S(-4, 0), S(0, 1), S(3, 1), S(5, 2), S(8, 1)},
    {S(-1, -11), S(0, -9), S(-1, -4), S(-2, 2), S(-1, 6), S(3, 4), S(-1, 7), S(4, 4)},
    {S(0, -5), S(0, -1), S(-2, 0), S(-5, 2), S(-2, 3), S(6, 2), S(4, 1), S(2, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(3, 0), S(-2, 0)},
    {S(-6, -3), S(-2, -1), S(0, 2), S(2, 3), S(2, 3), S(2, 1), S(1, -1), S(-2, -4)},
    {S(-3, -2), 0, S(1, 0), S(0, 1), S(0, 2), S(0, 1), S(3, 0), S(-1, -2)},
    {S(-1, 0), S(-2, 1), S(-1, 1), 0, S(1, -1), S(2, 0), S(2, 0), S(-1, -1)},
    {S(-3, -5), S(-2, -2), 0, S(0, 1), S(0, 3), S(1, 3), S(3, 0), S(2, 0)},
    {S(-2, -3), S(1, -1), S(-3, 1), S(-5, 2), S(-5, 2), S(-2, 1), S(2, 0), S(1, -3)},
};
const i32 open_files[][3] = {
    {S(28, 19), S(7, 16), S(-25, 7)},
    {S(59, 10), S(-6, 38), S(-71, -3)},
};
const i32 pawn_protection[] = {S(26, 13), S(5, 16), S(2, 7), S(10, 4), S(-9, 12), S(-35, 24)};
const i32 passers[] = {S(-9, 13), S(15, 43), S(47, 107), S(196, 187)};
const i32 pawn_passed_protected = S(16, 18);
const i32 pawn_doubled = S(-16, -32);
const i32 pawn_phalanx = S(12, 12);
const i32 pawn_passed_blocked[] = {S(-9, -19), S(9, -42), S(10, -79), S(20, -103)};
