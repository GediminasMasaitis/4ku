...
const i32 max_material[] = {157, 525, 559, 970, 1890, 0, 0};
const i32 material[] = {S(92, 157), S(333, 525), S(348, 559), S(424, 970), S(817, 1890), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(-1, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-5, -6), S(-3, -2), S(-1, 0), S(1, 4), S(4, 5), S(9, 2), S(5, -1), S(-11, -1)},
    {S(-4, -4), S(0, -2), S(1, 0), S(1, 2), S(2, 3), S(5, 1), S(1, 0), S(-7, 0)},
    {S(-1, -2), S(-3, -4), S(-4, -3), S(-4, 0), S(-1, 2), S(2, 2), S(4, 3), S(7, 2)},
    {S(0, -12), S(1, -10), S(0, -6), S(-1, 2), S(-1, 7), S(2, 5), S(-2, 8), S(2, 6)},
    {S(0, -6), S(0, -1), S(-3, 1), S(-6, 3), S(-3, 5), S(3, 4), S(1, 3), S(1, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(3, 0), S(-2, 0)},
    {S(-5, -4), S(-2, -1), S(0, 3), S(2, 4), S(2, 4), S(2, 1), S(1, -1), S(-1, -5)},
    {S(-3, -2), 0, S(1, 0), S(0, 2), S(0, 2), S(0, 1), S(2, -1), S(-1, -3)},
    {S(-1, 0), S(-2, 1), S(-1, 1), 0, S(1, -1), S(2, 0), S(2, 0), S(0, -2)},
    {S(-3, -6), S(-2, -3), S(-1, 0), S(0, 1), S(0, 4), S(1, 4), S(3, 1), S(3, 0)},
    {S(-2, -4), S(1, -1), S(-3, 1), S(-6, 2), S(-5, 2), S(-3, 1), S(1, 0), S(1, -4)},
};
const i32 open_files[][5] = {
    {S(1, 3), S(-6, 19), S(25, 23), S(5, 18), S(-25, 8)},
    {S(-4, -14), S(-11, -3), S(54, 17), S(-7, 45), S(-69, -5)},
};
const i32 pawn_protection[] = {S(24, 15), S(5, 19), S(2, 7), S(11, 5), S(-10, 16), S(-37, 29)};
const i32 passers[] = {S(-3, 16), S(19, 56), S(50, 137), S(196, 247)};
const i32 pawn_passed_protected = S(13, 22);
const i32 pawn_doubled = S(-13, -43);
const i32 pawn_phalanx = S(11, 14);
const i32 pawn_passed_blocked[] = {S(-9, -17), S(9, -45), S(11, -92), S(17, -115)};
const i32 pawn_passed_king_distance[] = {S(2, -7), S(-5, 13)};
const i32 bishop_pair = S(31, 79);
const i32 king_shield[] = {S(41, -10), S(32, -10)};
