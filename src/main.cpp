...
const i32 max_material[] = {134, 444, 447, 838, 1699, 0, 0};
const i32 material[] = {S(83, 134), S(325, 444), S(334, 447), S(440, 838), S(744, 1699), 0};
const i32 pst_rank[] = {
    ...
    S(-1, -5), S(1, -2),  0,         S(-2, 2),  S(0, 4),  S(5, 4), S(3, 3),  S(2, -3)    // King
};
const i32 pst_file[] = {
    S(0, 1),   S(-1, 1),  0,        S(2, -1), 0,        0,        S(1, 0),  S(-2, -1),  // Pawn
    ...
    S(-2, -4), S(3, -1),  S(-1, 1), S(-2, 2), S(-3, 2), S(-1, 1), S(2, -1), S(0, -4)    // King
};
...
const i32 king_attacks[] = {S(10, -5), S(19, -5), S(25, -9), S(20, 4), 0};
const i32 pawn_protection[] = {S(22, 14), S(2, 14), S(6, 17), S(8, 9), S(-5, 19), S(-35, 22)};
const i32 pawn_threat_penalty[] = {S(-4, 1), S(21, 2), S(12, 6), S(10, 18), S(9, 17), S(7, 7)};
const i32 passers[] = {S(0, 11), S(29, 46), S(56, 117), S(184, 248)};
const i32 pawn_passed_protected = S(10, 18);
const i32 pawn_doubled_penalty = S(10, 37);
const i32 pawn_phalanx = S(12, 11);
const i32 pawn_passed_blocked_penalty[] = {S(8, 13), S(-7, 40), S(-9, 81), S(-12, 159)};
const i32 pawn_passed_king_distance[] = {S(1, -6), S(-3, 11)};
const i32 bishop_pair = S(32, 71);
const i32 king_side_pawns = S(23, 4);
const i32 king_shield[] = {S(32, -11), S(25, -7)};
        ...
        for (i32 p = Pawn; p < None; ++p) {
            ...
            while (copy) {
                ...
                if (p == Pawn) {
                    ...
                    if (p == King) {
                        // King side pawns
                        score += king_side_pawns * count(pawns[0] & 0x0F0F0F0F0F0F0F0F << (file > 3) * 4);
