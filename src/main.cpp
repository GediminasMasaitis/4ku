...
const int max_material[] = {123, 396, 423, 738, 1468, 0, 0};
const int material[] = {S(123, 116), S(394, 396), S(410, 423), S(518, 738), S(918, 1468), 0};
const int pst_rank[][8] = {
    {0, S(-7, 1), S(-8, 0), S(-2, -2), S(3, -1), S(13, 2), 0, 0},
    {S(-15, -9), S(-10, -3), S(-3, 1), S(2, 6), S(10, 7), S(22, 2), S(12, -1), S(-19, -2)},
    {S(-11, -5), S(-2, -2), S(1, 0), S(3, 3), S(6, 4), S(14, 0), S(6, -1), S(-17, 0)},
    {S(-5, -4), S(-7, -6), S(-9, -4), S(-9, 1), S(-2, 3), S(7, 4), S(10, 5), S(16, 2)},
    {S(-6, -17), S(-1, -17), S(-1, -11), S(-4, 4), S(-3, 13), S(6, 8), S(0, 13), S(8, 8)},
    {S(2, -10), S(1, -2), S(-6, 2), S(-15, 7), S(-7, 9), S(3, 10), S(3, 7), S(5, -4)},
};
const int pst_file[][8] = {
    {S(-4, 2), S(-3, 2), S(-2, 0), S(0, -2), S(3, -1), S(4, 0), S(7, 0), S(-4, -1)},
    {S(-13, -5), S(-5, -1), S(1, 3), S(5, 5), S(6, 5), S(6, 2), S(3, -2), S(-4, -7)},
    {S(-6, -2), S(-1, 0), S(1, 0), S(1, 2), S(2, 3), S(1, 2), S(5, -1), S(-2, -4)},
    {S(-4, 1), S(-5, 2), S(-2, 2), S(1, 0), S(4, -2), S(4, 0), S(6, -1), S(-3, -1)},
    {S(-9, -7), S(-5, -2), S(-2, 2), S(0, 3), S(0, 5), S(3, 5), S(7, -1), S(6, -6)},
    {S(-3, -6), S(3, -2), S(-6, 2), S(-6, 3), S(-10, 4), S(-2, 2), S(4, 0), S(0, -5)},
};
const int open_files[][3] = {
    {S(31, 17), S(10, 13), S(-32, 11)},
    {S(65, 7), S(-11, 43), S(-87, 1)},
};
const int pawn_protection[] = {S(30, 12), S(7, 11), S(4, 6), S(11, 2), S(-10, 12), S(-31, 21)};
const int passers[] = {S(5, 11), S(42, 38), S(85, 95), S(290, 164)};
const int pawn_passed_protected = S(16, 19);
const int pawn_doubled = S(-20, -30);
const int pawn_phalanx = S(13, 11);
const int pawn_passed_blocked[] = {S(-12, -18), S(7, -42), S(12, -75), S(-20, -83)};
const int pawn_passed_king_distance[] = {S(0, -5), S(-4, 9)};
const int bishop_pair = S(41, 61);
const int king_shield[] = {S(49, -12), S(37, -10)};
