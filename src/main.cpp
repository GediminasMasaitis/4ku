...
const int max_material[] = {124, 393, 430, 745, 1347, 0, 0};
const int material[] = {S(90, 124), S(382, 393), S(403, 430), S(511, 745), S(1164, 1347), 0};
const int pst_rank[][8] = {
    {0, S(-6, 1), S(-5, -1), S(-1, -3), S(4, -1), S(8, 5), 0, 0},
    {S(-9, -8), S(-4, -3), 0, S(4, 7), S(9, 8), S(18, 1), S(10, -3), S(-28, -3)},
    {S(-8, -5), S(-1, -4), S(2, -1), S(3, 3), S(5, 4), S(10, 1), S(2, 0), S(-13, 0)},
    {S(-5, -2), S(-9, -4), S(-8, -4), S(-7, 1), S(0, 2), S(6, 2), S(9, 3), S(14, 3)},
    {S(-2, -15), S(0, -21), S(0, -11), S(-2, 4), S(-2, 12), S(5, 8), S(-3, 13), S(4, 9)},
    {S(-4, -7), S(-4, 1), S(-3, 2), S(-4, 5), S(1, 6), S(21, 3), S(11, 2), S(-4, -5)},
};
const int pst_file[][8] = {
    ...
    {S(-5, -11), S(-1, -8), S(0, -4), S(-1, 5), S(-2, 6), S(1, 6), S(6, 1), S(3, 4)},
    {S(-2, -6), S(4, -2), S(-4, 2), S(-12, 5), S(-9, 4), S(-5, 3), S(3, 1), S(5, -7)},
};
const int open_files[][3] = {
    {S(25, 17), S(2, 27), S(-19, 3)},
    {S(51, 12), S(-1, 30), S(-52, -8)},
};
const int pawn_protection[] = {S(20, 17), S(4, 20), S(0, 11), S(8, 8), S(-9, 16), S(-34, 22)};
const int passers[] = {S(-23, 17), S(-12, 49), S(8, 116), S(137, 236)};
const int pawn_passed_protected = S(14, 16);
const int pawn_doubled = S(-15, -26);
const int pawn_phalanx = S(10, 16);
const int pawn_passed_blocked[] = {S(-6, -17), S(8, -36), S(-2, -68), S(26, -109)};
const int pawn_passed_king_distance[] = {S(3, -6), S(-3, 10)};
const int bishop_pair = S(23, 68);
const int king_shield[] = {S(32, -7), S(24, -6)};
