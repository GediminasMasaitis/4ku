...
const i32 max_material[] = {138, 436, 438, 819, 1626, 0, 0};
const i32 material[] = {S(101, 138), S(334, 436), S(347, 438), S(463, 819), S(848, 1626), 0};
const i32 pst_rank[] = {
    0,         S(-2, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(5, 2), 0,        0,          // Pawn
    S(-4, -5), S(-2, -2), S(-1, -1), S(1, 3),   S(3, 3),  S(8, 1), S(5, 0),  S(-11, 1),  // Knight
    S(-2, -2), S(1, -1),  S(1, 0),   S(1, 0),   S(2, 1),  S(4, 0), S(1, 0),  S(-8, 2),   // Bishop
    S(-2, -3), S(-2, -4), S(-3, -2), S(-4, 1),  S(-1, 2), S(3, 2), S(3, 3),  S(6, 1),    // Rook
    S(1, -13), S(1, -10), S(0, -5),  S(-1, 1),  S(-1, 6), S(1, 5), S(-2, 8), S(1, 7),    // Queen
    S(0, -6),  S(0, -2),  S(-2, 0),  S(-4, 3),  S(-1, 4), S(5, 4), S(2, 3),  S(2, -4)    // King
};
const i32 pst_file[] = {
    S(-2, 1),  S(-1, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, -1),  // Pawn
    S(-4, -2), S(-2, -1), S(0, 1),  S(2, 3),  S(2, 2),  S(2, 0),  S(1, -1), S(-1, -3),  // Knight
    S(-2, 0),  0,         S(1, 0),  S(0, 1),  S(0, 1),  S(-1, 1), S(2, -1), S(0, -1),   // Bishop
    S(-2, 0),  S(-2, 1),  S(-1, 1), S(1, 0),  S(1, -1), S(1, 0),  S(2, 0),  S(-1, -1),  // Rook
    S(-3, -4), S(-2, -2), S(-1, 0), S(0, 1),  S(0, 2),  S(1, 3),  S(2, 2),  S(3, -1),   // Queen
    S(-2, -5), S(2, -1),  S(-2, 1), S(-3, 2), S(-4, 2), S(-1, 1), S(2, -1), S(0, -5)    // King
};
const i32 open_files[] =
    {S(2, 4), S(-4, 21), S(20, 17), S(3, 19), S(-23, 10), S(-3, -12), S(-11, 0), S(47, -1), S(-14, 35), S(-63, 1)};
const i32 mobilities[] = {S(9, 5), S(8, 7), S(4, 4), S(4, 3), S(-5, 0)};
const i32 pawn_protection[] = {S(24, 14), S(2, 15), S(8, 17), S(9, 9), S(-5, 23), S(-34, 26)};
const i32 passers[] = {S(-1, 15), S(27, 51), S(58, 124), S(187, 251)};
const i32 pawn_passed_protected = S(13, 19);
const i32 pawn_doubled_penalty = S(14, 38);
const i32 pawn_phalanx = S(13, 12);
const i32 pawn_passed_blocked_penalty[] = {S(11, 13), S(-5, 42), S(-8, 84), S(-11, 162)};
const i32 pawn_passed_king_distance[] = {S(1, -6), S(-4, 11)};
const i32 bishop_pair = S(31, 71);
