...
const i32 max_material[] = {147, 520, 523, 956, 1783, 0, 0};
const i32 material[] = {S(89, 147), S(349, 520), S(362, 523), S(476, 956), S(1046, 1783), 0};
const i32 pst_rank[] = {
    0,         S(-3, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(5, 3),  0,        0,          // Pawn
    S(-1, -5), S(0, -3),  S(1, -1),  S(3, 3),   S(4, 4),  S(5, 1),  S(2, 0),  S(-15, 1),  // Knight
    ...
    for (i32 c = 0; c < 2; ++c) {
        ...
        for (i32 p = Pawn; p < None; ++p) {
            ...
            while (copy) {
                ...
                // Pawn threat
                const u64 file_bb = 0x101010101010101ull << sq;
                if (file_bb & ~piece_bb & attacked_by_pawns)
                    score -= pawn_threat_penalty[p];

                if (p == Pawn) {
                    // Passed pawns
                    if (rank > 2 && !(file_bb & (pawns[1] | attacked_by_pawns))) {
                    ...
                    // Doubled rooks
                        ...
                        score += rook_doubled * count(file_bb & ~piece_bb & pos.colour[0] & pos.pieces[Rook]);
                    }
