...
const i32 max_material[] = {143, 470, 473, 874, 1747, 0, 0};
const i32 material[] = {S(98, 143), S(311, 470), S(321, 473), S(424, 874), S(751, 1747), 0};
const i32 pst_rank[][8] = {
    {0, S(-2, 0), S(-3, -1), S(-1, -1), S(1, 0), S(5, 2), 0, 0},
    {S(-4, -5), S(-2, -2), S(-1, -1), S(1, 3), S(3, 4), S(7, 1), S(5, 0), S(-10, 1)},
    {S(-2, -2), S(1, -2), S(1, 0), S(1, 0), S(2, 1), S(4, 0), S(1, 0), S(-8, 2)},
    {S(-1, -4), S(-2, -4), S(-3, -2), S(-4, 1), S(-1, 2), S(3, 2), S(3, 3), S(5, 1)},
    {S(1, -13), S(1, -10), S(0, -5), S(-1, 1), S(-2, 6), S(1, 6), S(-2, 8), S(1, 6)},
    {S(1, -6), S(0, -2), S(-2, 0), S(-4, 3), S(-1, 5), S(4, 4), S(3, 3), S(4, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 1), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 0), S(-1, -1)},
    {S(-4, -3), S(-2, -1), S(0, 1), S(2, 3), S(2, 2), S(2, 0), S(1, -1), S(-1, -3)},
    {S(-2, 0), 0, S(1, 0), S(0, 1), S(0, 1), S(-1, 1), S(1, -1), S(0, -1)},
    {S(-2, 0), S(-2, 1), S(-1, 1), S(1, 0), S(1, -1), S(1, 0), S(2, 0), S(-1, -1)},
    {S(-3, -4), S(-2, -2), S(-1, 0), S(0, 1), S(0, 2), S(1, 3), S(2, 1), S(3, 0)},
    {S(-2, -4), S(2, -1), S(-2, 1), S(-3, 2), S(-4, 2), S(-1, 1), S(2, -1), S(0, -5)},
};
const i32 open_files[][5] = {
    {S(3, 4), S(-4, 21), S(20, 16), S(3, 20), S(-21, 9)},
    {S(-4, -12), S(-11, 0), S(47, 0), S(-13, 34), S(-62, 1)},
};
const i32 mobilities[] = {S(9, 5), S(8, 7), S(4, 4), S(4, 4), S(-5, -1)};
const i32 pawn_protection[] = {S(23, 15), S(2, 16), S(8, 18), S(9, 8), S(-6, 24), S(-33, 25)};
const i32 passers[] = {S(1, 15), S(30, 53), S(64, 128), S(209, 217)};
const i32 pawn_passed_protected = S(12, 20);
const i32 pawn_doubled = S(-14, -39);
const i32 pawn_phalanx = S(12, 13);
const i32 pawn_passed_blocked[] = {S(-10, -14), S(6, -44), S(11, -91), S(-1, -108)};
const i32 pawn_passed_king_distance[] = {S(1, -7), S(-4, 11)};
const i32 bishop_pair = S(30, 73);
const i32 king_shield[] = {S(35, -11), S(28, -8)};
