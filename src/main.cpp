...
const i32 max_material[] = {147, 417, 443, 773, 1463, 0, 0};
const i32 material[] = {S(95, 147), S(374, 417), S(372, 443), S(488, 773), S(979, 1463), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(-1, -1), S(1, 0), S(5, 2), 0, 0},
    {S(-6, -6), S(-4, -2), S(-1, 1), S(1, 4), S(4, 4), S(10, 1), S(6, -1), S(-11, -2)},
    {S(-1, -3), S(0, -2), S(-1, 0), S(-1, 2), S(1, 2), S(5, 1), S(2, 0), S(-6, 0)},
    {S(-2, -2), S(-4, -3), S(-5, -2), S(-4, 1), S(-1, 2), S(3, 2), S(5, 2), S(8, 1)},
    {S(-2, -10), S(0, -8), S(0, -5), S(-1, 1), S(-1, 5), S(3, 4), S(-1, 7), S(3, 5)},
    {S(-1, -5), S(0, -1), S(-1, 0), S(-4, 2), S(0, 3), S(7, 2), S(4, 1), S(1, -3)},
};
const i32 pst_file[][8] = {
    {S(0, -2), S(-1, 0), S(-2, 1), S(-1, 1), S(0, 2), S(2, 1), S(3, 0), S(-1, -3)},
    {S(-5, -4), S(-2, -1), S(0, 3), S(2, 4), S(2, 4), S(2, 1), S(1, -1), S(-1, -5)},
    {S(1, -2), S(-1, 0), S(-1, 0), S(-1, 2), S(-1, 2), S(-1, 1), S(2, 0), S(3, -3)},
    {S(-1, 0), S(-2, 1), S(-1, 1), S(0, 1), S(1, 0), S(2, 0), S(2, -1), S(0, -1)},
    {S(-3, -4), S(-1, -2), S(0, -1), S(0, 1), S(0, 2), S(1, 3), S(3, 1), S(2, 1)},
    {S(-2, -3), S(1, -1), S(-3, 1), S(-4, 2), S(-5, 2), S(-2, 1), S(1, 0), S(1, -3)},
};
const i32 centralities[] = {S(9, -15), S(4, -6), S(25, -1), S(5, -4), S(-2, 9), S(-10, 3)};
const i32 open_files[][5] = {
    {S(0, 3), S(-8, 16), S(28, 19), S(6, 17), S(-26, 8)},
    {S(-4, -12), S(-13, -2), S(58, 10), S(-6, 38), S(-70, -4)},
};
const i32 pawn_protection[] = {S(26, 13), S(6, 15), S(2, 5), S(9, 5), S(-8, 11), S(-35, 23)};
const i32 passers[] = {S(-9, 12), S(15, 42), S(44, 106), S(193, 191)};
const i32 pawn_passed_protected = S(15, 18);
const i32 pawn_doubled = S(-15, -33);
const i32 pawn_phalanx = S(12, 13);
const i32 pawn_passed_blocked[] = {S(-7, -12), S(7, -35), S(10, -75), S(19, -99)};
const i32 pawn_passed_king_distance[] = {S(2, -6), S(-4, 10)};
const i32 bishop_pair = S(35, 65);
const i32 king_shield[] = {S(43, -11), S(30, -8)};
    ...
    for (i32 c = 0; c < 2; ++c) {
        ...
        for (i32 p = 0; p < 6; ++p) {
            ...
            while (copy) {
                ...
                // Centrality
                score += centralities[p] * (7 - abs(7 - rank - file) - abs(rank - file)) / 2;
