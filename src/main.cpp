...
const int max_material[] = {123, 393, 430, 746, 1348, 0, 0};
const int material[] = {S(92, 123), S(384, 393), S(405, 430), S(516, 746), S(1174, 1348), 0};
const int pst_rank[][8] = {
    {0, S(-6, 0), S(-5, -1), S(-1, -3), S(4, -1), S(8, 5), 0, 0},
    {S(-9, -8), S(-4, -3), 0, S(4, 7), S(9, 8), S(18, 1), S(9, -3), S(-28, -3)},
    {S(-8, -4), S(-1, -4), S(2, 0), S(3, 3), S(5, 4), S(10, 1), S(2, 0), S(-14, 0)},
    {S(-5, -2), S(-9, -4), S(-8, -4), S(-7, 1), S(0, 2), S(6, 2), S(9, 3), S(14, 3)},
    {S(-3, -14), S(0, -20), S(0, -11), S(-1, 4), S(-1, 11), S(5, 8), S(-4, 13), S(4, 9)},
    {S(-3, -8), S(-5, 1), S(-3, 2), S(-5, 5), S(0, 6), S(20, 4), S(11, 2), S(-1, -5)},
};
const int pst_file[][8] = {
    {S(-4, 1), S(-3, 2), S(-2, 0), S(1, -3), S(2, 0), S(4, 0), S(5, 0), S(-4, 0)},
    {S(-9, -9), S(-2, -2), S(1, 3), S(3, 7), S(3, 6), S(4, 3), S(2, -1), S(-3, -6)},
    {S(-5, -4), S(1, -1), S(0, 1), S(0, 2), S(0, 3), S(0, 2), S(4, 2), S(0, -4)},
    {S(-2, 0), S(-3, 1), S(-2, 2), 0, S(1, 0), S(2, 0), S(3, -1), S(1, -3)},
    {S(-5, -11), S(-1, -8), S(0, -3), S(-1, 5), S(-2, 6), S(1, 5), S(6, 1), S(3, 4)},
    {S(-3, -6), S(2, -2), S(-7, 3), S(-15, 5), S(2, 1), S(-8, 4), S(1, 1), S(3, -7)},
};
const int open_files[][3] = {
    {S(25, 17), S(2, 27), S(-15, 2)},
    {S(52, 11), S(-1, 29), S(-51, -8)},
};
const int pawn_protection[] = {S(21, 17), S(3, 20), S(0, 11), S(8, 8), S(-10, 16), S(-38, 24)};
const int passers[] = {S(-23, 16), S(-13, 49), S(7, 115), S(132, 235)};
const int pawn_passed_protected = S(14, 17);
const int pawn_doubled = S(-15, -27);
const int pawn_phalanx = S(12, 15);
const int pawn_passed_blocked[] = {S(-6, -16), S(6, -35), S(-2, -68), S(27, -107)};
const int pawn_passed_king_distance[] = {S(3, -6), S(-3, 9)};
const int bishop_pair = S(23, 69);
const int king_shield[] = {S(29, -4), S(23, -5)};
        ...
        for (int p = 0; p < 6; ++p) {
            ...
            while (copy) {
                ...
                if (p == Pawn) {
                    ...
                    if (p == King && piece_bb & 0xC3C7) {
                        // C3C7 = Reasonable king squares
