...
const i32 max_material[] = {125, 385, 389, 719, 1387, 0, 0};
const i32 material[] = {S(106, 125), S(355, 385), S(367, 389), S(498, 719), S(970, 1387), 0};
const i32 pst_rank[] = {0, S(-2, 0), S(-3, -1), S(-1, -1), S(1, 0), S(5, 2), 0, 0, // Pawn
S(-4, -4), S(-2, -2), S(-1, -1), S(1, 3), S(3, 3), S(8, 1), S(5, 0), S(-11, 0), // Knight
S(-2, -1), S(1, -1), S(1, 0), S(1, 0), S(2, 1), S(4, 0), S(1, 0), S(-8, 1), // Bishop
S(-2, -3), S(-3, -3), S(-4, -2), S(-4, 1), S(-1, 2), S(3, 2), S(4, 2), S(6, 1), // Rook
S(1, -12), S(1, -9), S(0, -4), S(-1, 1), S(-2, 6), S(1, 5), S(-2, 8), S(2, 5), // Queen
S(0, -5), S(0, -1), S(-1, 0), S(-4, 2), S(0, 3), S(7, 2), S(4, 2), S(1, -3)// King
};
const i32 pst_file[] = {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 0), S(-2, 0), // Pawn
S(-5, -2), S(-2, -1), S(0, 1), S(2, 3), S(2, 2), S(2, 0), S(1, -1), S(-1, -3), // Knight
S(-2, 0), 0, S(1, 0), S(0, 1), S(0, 1), S(-1, 1), S(2, -1), S(0, -1), // Bishop
S(-2, 0), S(-2, 1), S(0, 1), S(1, 0), S(1, -1), S(1, 0), S(2, 0), S(-1, -1), // Rook
S(-3, -4), S(-2, -3), S(-1, -1), 0, S(0, 2), S(1, 3), S(3, 1), S(3, 1), // Queen
S(-3, -4), S(2, -2), S(-2, 1), S(-4, 2), S(-4, 2), S(-2, 1), S(2, -1), S(0, -4)// King
};
const i32 open_files[] = {S(4, 3), S(-4, 18), S(21, 16), S(4, 17), S(-21, 8), S(-2, -12), S(-9, -2), S(50, -2), S(-11, 26), S(-63, 1)};
const i32 mobilities[] = {S(9, 4), S(9, 6), S(4, 4), S(4, 4), S(-4, -1)};
const i32 pawn_protection[] = {S(25, 13), S(2, 16), S(9, 16), S(10, 9), S(-6, 23), S(-39, 23)};
const i32 passers[] = {S(-5, 12), S(22, 43), S(53, 108), S(174, 230)};
const i32 pawn_passed_protected = S(13, 17);
const i32 pawn_doubled_penalty = S(16, 33);
const i32 pawn_phalanx = S(13, 12);
const i32 pawn_passed_blocked_penalty[] = {S(11, 11), S(-5, 35), S(-4, 75), S(-28, 160)};
const i32 pawn_passed_king_distance[] = {S(2, -6), S(-4, 10)};
const i32 bishop_pair = S(32, 65);
const i32 king_shield[] = {S(37, -11), S(29, -8)};
