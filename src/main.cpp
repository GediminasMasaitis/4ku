...
const int max_material[] = {124, 389, 427, 740, 1337, 0, 0};
const int material[] = {S(91, 124), S(385, 389), S(403, 427), S(513, 740), S(1167, 1337), 0};
const int pst_rank[][8] = {
    {0, S(-6, 0), S(-5, -1), S(0, -3), S(4, -1), S(8, 5), 0, 0},
    {S(-9, -8), S(-4, -3), 0, S(4, 7), S(9, 8), S(19, 1), S(10, -3), S(-29, -3)},
    {S(-8, -4), S(-1, -4), S(2, -1), S(3, 3), S(5, 4), S(11, 1), S(2, 0), S(-14, 0)},
    {S(-5, -2), S(-10, -4), S(-8, -4), S(-7, 1), S(1, 2), S(6, 1), S(9, 3), S(14, 3)},
    {S(-2, -14), S(0, -20), S(0, -11), S(-2, 4), S(-1, 11), S(5, 8), S(-4, 13), S(4, 9)},
    {S(-4, -7), S(-4, 1), S(-2, 2), S(-4, 5), S(1, 5), S(21, 3), S(12, 2), S(-4, -5)},
};
const int pst_file[][8] = {
    {S(-3, 1), S(-2, 1), S(-2, 0), S(1, -3), S(2, 0), S(4, 0), S(4, 1), S(-4, 0)},
    {S(-9, -9), S(-2, -2), S(1, 3), S(3, 7), S(4, 6), S(4, 3), S(2, -1), S(-3, -6)},
    {S(-5, -4), S(0, -1), S(1, 1), S(0, 2), S(0, 3), S(0, 2), S(4, 2), S(0, -4)},
    {S(-2, 0), S(-3, 1), S(-1, 2), S(1, 0), S(1, 0), S(2, 0), S(2, 0), S(0, -2)},
    {S(-5, -10), S(-1, -8), S(0, -4), S(-1, 5), S(-2, 6), S(1, 5), S(6, 1), S(3, 5)},
    {S(-1, -6), S(4, -2), S(-4, 2), S(-11, 4), S(-9, 4), S(-5, 3), S(3, 0), S(5, -7)},
};
const int open_files[][3] = {
    {S(25, 17), S(2, 28), S(-19, 4)},
    {S(51, 12), S(-1, 31), S(-52, -8)},
};
const int pawn_protection[] = {S(20, 17), S(3, 20), S(0, 11), S(7, 8), S(-10, 16), S(-35, 22)};
const int passers[] = {S(-23, 17), S(-12, 49), S(10, 116), S(131, 236)};
const int pawn_passed_protected = S(14, 16);
const int pawn_doubled = S(-15, -26);
const int pawn_phalanx = S(10, 15);
const int pawn_passed_blocked[] = {S(-7, -16), S(8, -37), S(-3, -69), S(39, -112)};
const int pawn_passed_king_distance[] = {S(3, -6), S(-4, 9)};
const int bishop_pair = S(24, 67);
const int king_shield[] = {S(32, -8), S(24, -6)};
