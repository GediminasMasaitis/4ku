...
const i32 max_material[] = {139, 448, 451, 842, 1678, 0, 0};
const i32 material[] = {S(102, 139), S(331, 448), S(343, 451), S(456, 842), S(826, 1678), 0};
const i32 pst_rank[][8] = {
    {0, S(-2, 0), S(-3, -1), S(-1, -1), S(1, 0), S(5, 2), 0, 0},
    {S(-4, -5), S(-2, -3), S(-1, -1), S(1, 3), S(3, 4), S(7, 1), S(6, 0), S(-11, 1)},
    {S(-2, -2), S(1, -1), S(1, 0), S(1, 0), S(2, 1), S(3, 0), S(1, 0), S(-8, 2)},
    {S(-1, -4), S(-2, -4), S(-3, -2), S(-3, 1), S(0, 2), S(3, 2), S(2, 4), S(4, 2)},
    {S(2, -12), S(2, -9), S(1, -4), S(-1, 1), S(-1, 6), S(0, 6), S(-2, 7), S(0, 6)},
    {S(0, -6), S(1, -2), S(-1, 0), S(-3, 3), S(-1, 4), S(5, 4), S(3, 3), S(2, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 1), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 0), S(-2, -1)},
    {S(-4, -3), S(-2, -1), S(0, 1), S(2, 3), S(2, 2), S(2, 0), 0, S(-1, -3)},
    {S(-2, 0), 0, S(1, 0), S(0, 1), S(0, 1), S(-1, 1), S(2, -1), S(0, -1)},
    {S(-2, 0), S(-1, 1), S(0, 1), S(1, 0), S(2, -1), S(1, 0), S(1, 0), S(-2, 0)},
    {S(-2, -4), S(-1, -2), S(-1, 0), S(0, 1), S(0, 2), S(1, 2), S(2, 1), S(2, -1)},
    {S(-2, -5), S(2, -2), S(-1, 1), S(-3, 2), S(-3, 2), S(-1, 1), S(2, -1), S(0, -5)},
};
const i32 open_files[][5] = {
    {S(3, 5), S(-4, 21), S(19, 16), S(4, 19), S(-22, 10)},
    {S(-3, -12), S(-11, 0), S(46, 0), S(-14, 38), S(-60, 0)},
};
const i32 mobilities[] = {S(9, 5), S(8, 7), S(3, 4), S(4, 2), S(-5, 0)};
const i32 king_attacks[] = {S(5, -2), S(16, -2), S(24, -9), S(18, 5), 0};
const i32 pawn_protection[] = {S(24, 14), S(2, 15), S(8, 17), S(9, 8), S(-5, 19), S(-33, 26)};
const i32 passers[] = {S(2, 14), S(33, 51), S(68, 124), S(218, 206)};
const i32 pawn_passed_protected = S(13, 20);
const i32 pawn_doubled_penalty = S(15, 38);
const i32 pawn_phalanx = S(13, 12);
const i32 pawn_passed_blocked_penalty[] = {S(11, 14), S(-5, 43), S(-9, 85), S(1, 100)};
        ...
        for (i32 p = 0; p < 6; ++p) {
            ...
            while (copy) {
                ...
                if (p == Pawn) {
                    ...
                    score += king_attacks[p - 1] *
                             count(mobility & ~pos.colour[0] & king(kings[1], pos.colour[0] | pos.colour[1]));
