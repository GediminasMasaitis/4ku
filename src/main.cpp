...
const i32 max_material[] = {136, 442, 446, 837, 1701, 0, 0};
const i32 material[] = {S(93, 136), S(325, 442), S(334, 446), S(440, 837), S(740, 1701), 0};
const i32 pst_rank[] = {
    ...
    S(0, -6),  S(0, -2),  0,         S(-2, 2),  S(0, 4),  S(6, 4), S(3, 3),  S(1, -3)    // King
};
const i32 pst_file[] = {
    S(-1, 1),  S(-2, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(1, 0),  S(2, 0), S(-1, -1),  // Pawn
    S(-5, -3), S(-2, -1), S(0, 1),  S(2, 3),  S(2, 2),  S(2, 0),  S(1, 0), S(-1, -3),  // Knight
    S(-2, 0),  0,         S(1, 0),  S(0, 1),  S(1, 1),  S(-1, 1), S(2, 0), S(0, -1),   // Bishop
    S(-2, 0),  S(-1, 1),  S(0, 1),  S(1, 0),  S(1, -1), 0,        S(1, 0), S(0, -1),   // Rook
    S(-2, -4), S(-1, -2), S(-1, 0), S(0, 1),  S(0, 2),  S(1, 2),  S(2, 1), S(1, -1),   // Queen
    S(-4, -4), S(0, -1),  S(-3, 1), S(-3, 2), S(4, 0),  S(-1, 1), 0,       S(-2, -4)   // King
};
...
const i32 king_attacks[] = {S(10, -4), S(20, -5), S(27, -9), S(20, 5), 0};
const i32 pawn_protection[] = {S(23, 14), S(2, 14), S(6, 17), S(8, 9), S(-6, 19), S(-31, 25)};
const i32 pawn_threat_penalty[] = {S(-4, 1), S(21, 2), S(11, 6), S(10, 18), S(9, 16), S(4, 6)};
const i32 passers[] = {S(3, 13), S(31, 50), S(60, 124), S(192, 254)};
const i32 pawn_passed_protected = S(9, 18);
const i32 pawn_doubled_penalty = S(11, 36);
const i32 pawn_phalanx = S(13, 11);
const i32 pawn_passed_blocked_penalty[] = {S(8, 13), S(-7, 40), S(-8, 81), S(-11, 160)};
const i32 pawn_passed_king_distance[] = {S(1, -6), S(-4, 11)};
const i32 bishop_pair = S(32, 71);
const i32 king_shield[] = {S(36, -8), S(30, -6), S(7, -2)};
            ...
            while (copy) {
                ...
                if (p == Pawn) {
                    ...
                    if (p == King && piece_bb & 0xC3C7) {
                        ...
                        u64 shield = 0x700 << 5 * (file > 2);
                        for (i32 i = 0; i < 2; ++i) {
                            score += count(shield & pawns[0]) * king_shield[i];
                            shield <<= 8;
                        }
                    }
                }
