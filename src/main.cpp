...
const i32 material[] = {S(107, 125), S(377, 407), S(391, 441), S(493, 766), S(976, 1467), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, 0), S(-1, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-6, -4), S(-4, -1), S(-1, 0), S(1, 3), S(5, 4), S(10, 1), S(6, -1), S(-11, -1)},
    ...
    {S(0, -6), S(0, -2), S(-2, 0), S(-6, 3), S(-2, 4), S(4, 4), S(2, 2), S(-1, -3)},
};
const i32 pst_file[][8] = {
    ...
    {S(-1, -4), S(2, -1), S(-2, 1), S(-4, 2), S(-5, 2), S(-2, 1), S(2, 0), S(2, -4)},
};
const i32 open_files[][3] = {
    {S(28, 18), S(6, 21), S(-26, 10)},
    {S(59, 10), S(-6, 37), S(-70, -4)},
};
const i32 pawn_protection[] = {S(26, 14), S(4, 17), S(1, 8), S(9, 5), S(-10, 13), S(-35, 22)};
const i32 passers[] = {S(-19, 39), S(39, 39), S(111, 78), S(304, 150)};
const i32 pawn_passed_protected = S(16, 15);
const i32 pawn_doubled = S(-16, -31);
const i32 pawn_phalanx = S(12, 13);
const i32 pawn_passed_blocked[] = {S(1, -41), S(13, -52), S(10, -73), S(13, -83)};
            ...
            while (copy) {
                ...
                if (p == Pawn) {
                    ...
                    if (rank > 2 && !(0x101010101010101ULL << sq & (pawns[1] | attacked_by_pawns))) {
                        ...
                        for (i32 i = 0; i < 2; ++i)
                            score += pawn_passed_king_distance[i] * (rank - 3) *
