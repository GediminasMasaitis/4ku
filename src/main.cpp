...
const i32 max_material[] = {125, 406, 440, 765, 1465, 0, 0};
const i32 material[] = {S(106, 125), S(378, 406), S(392, 440), S(493, 765), S(978, 1465), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(-1, -1), S(2, -1), S(5, 2), 0, 0},
    {S(-6, -5), S(-4, -1), S(-1, 0), S(1, 3), S(5, 4), S(10, 1), S(6, -1), S(-11, -1)},
    {S(-5, -2), S(-1, -1), S(1, 0), S(2, 1), S(3, 2), S(6, 0), S(2, 0), S(-7, 0)},
    {S(-3, -2), S(-4, -3), S(-4, -2), S(-4, 0), S(0, 1), S(3, 1), S(5, 2), S(7, 1)},
    {S(-2, -8), S(0, -9), S(0, -5), S(-2, 2), S(-1, 6), S(3, 4), S(-1, 7), S(4, 4)},
    {S(0, -5), S(0, -1), S(-1, 0), S(-5, 2), S(-2, 3), S(6, 2), S(4, 1), S(2, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(3, 0), S(-2, 0)},
    {S(-5, -4), S(-2, -1), S(1, 2), S(2, 3), S(2, 3), S(3, 1), S(1, -1), S(-2, -4)},
    ...
    {S(-1, -3), S(2, -1), S(-2, 1), S(-4, 2), S(-5, 2), S(-1, 1), S(2, 0), S(1, -3)},
};
const i32 open_files[][3] = {
    {S(28, 18), S(6, 21), S(-25, 7)},
    {S(59, 10), S(-6, 38), S(-70, -4)},
};
const i32 pawn_protection[] = {S(26, 14), S(5, 16), S(2, 8), S(10, 4), S(-10, 13), S(-33, 23)};
const i32 passers[] = {S(-10, 15), S(14, 45), S(46, 107), S(198, 194)};
const i32 pawn_passed_protected = S(16, 18);
const i32 pawn_doubled = S(-17, -30);
const i32 pawn_phalanx = S(12, 13);
const i32 pawn_passed_blocked[] = {S(-7, -20), S(10, -42), S(13, -78), S(20, -101)};
const i32 pawn_passed_king_distance[] = {S(2, -6), S(-4, 10)};
const i32 bishop_pair = S(35, 65);
const i32 king_shield[] = {S(46, -12), S(35, -9), S(10, -9)};
