...
const i32 max_material[] = {143, 443, 435, 828, 1647, 0, 0};
const i32 material[] = {S(92, 143), S(341, 443), S(354, 435), S(469, 828), S(853, 1647), 0};
const i32 pst_rank[] = {
    0,         S(-3, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(5, 2), 0,        0,          // Pawn
    S(-3, -4), S(-1, -2), 0,         S(2, 2),   S(4, 3),  S(6, 1), S(4, 0),  S(-12, 1),  // Knight
    S(-1, -1), S(2, -1),  S(2, 0),   S(2, 0),   S(2, 1),  S(3, 0), 0,        S(-8, 2),   // Bishop
    S(0, -3),  S(-1, -3), S(-2, -2), S(-3, 1),  S(0, 1),  S(2, 1), S(1, 3),  S(3, 2),    // Rook
    S(2, -11), S(2, -9),  S(1, -4),  S(0, 1),   S(-1, 5), S(0, 5), S(-3, 7), S(-1, 6),   // Queen
    S(-1, -5), S(1, -2),  0,         S(-2, 2),  S(1, 4),  S(6, 4), S(3, 2),  S(1, -4)    // King
};
const i32 pst_file[] = {
    S(-1, 1),  S(-2, 1),  S(-1, 0), S(0, -1), S(1, 0),  S(2, 0),  S(2, 0),  S(-1, -1),  // Pawn
    S(-4, -3), S(-1, -1), S(0, 1),  S(2, 3),  S(2, 3),  S(2, 1),  S(1, 0),  S(-1, -3),  // Knight
    S(-2, 0),  0,         S(1, 0),  S(0, 1),  S(1, 1),  S(-1, 1), S(2, 0),  S(0, -1),   // Bishop
    S(-2, 0),  S(-1, 1),  S(0, 1),  S(1, 0),  S(2, -1), S(1, 0),  S(1, 0),  S(-2, 0),   // Rook
    S(-2, -4), S(-1, -2), S(-1, 0), S(0, 1),  S(0, 2),  S(1, 3),  S(2, 1),  S(2, -1),   // Queen
    S(-3, -5), S(3, -2),  S(-1, 1), S(-2, 1), S(-3, 2), S(-1, 1), S(2, -1), S(0, -4)    // King
};
...
const i32 mobilities[] = {S(9, 4), S(7, 7), S(3, 4), S(4, 2), S(-5, -1)};
const i32 king_attacks[] = {S(13, -7), S(19, -6), S(27, -11), S(22, 2), 0};
const i32 unprotected_pawn_attacks[] = {S(-13, 23), S(-3, 24), S(-3, 18), S(-5, 3), S(-3, 5)};
const i32 pawn_protection[] = {S(25, 9), S(2, 13), S(6, 19), S(8, 8), S(-5, 18), S(-30, 25)};
const i32 pawn_threat_penalty[] = {S(-4, 2), S(16, 9), S(12, 3), S(11, 13), S(11, 15), S(7, 4)};
const i32 passers[] = {S(5, 13), S(35, 49), S(67, 122), S(203, 249)};
const i32 pawn_passed_protected = S(10, 18);
const i32 pawn_doubled_penalty = S(10, 38);
const i32 pawn_phalanx = S(13, 10);
const i32 pawn_passed_blocked_penalty[] = {S(10, 13), S(-6, 40), S(-7, 82), S(-10, 155)};
const i32 pawn_passed_king_distance[] = {S(1, -6), S(-4, 11)};
const i32 bishop_pair = S(32, 72);
const i32 king_shield[] = {S(35, -11), S(26, -6)};
            ...
            while (copy) {
                ...
                if (p == Pawn) {
                    ...
                    // King
                        ...
                        mobility = king(sq, pos.colour[0] | pos.colour[1]);
                    ...
                    // Attacks on unprotected pawns
                    score += unprotected_pawn_attacks[p - 1] * count(mobility & pawns[1] & ~attacked_by_pawns);
