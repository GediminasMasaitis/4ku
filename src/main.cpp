...
const i32 max_material[] = {126, 404, 441, 766, 1463, 0, 0};
const i32 material[] = {S(106, 126), S(374, 404), S(389, 441), S(488, 766), S(971, 1463), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(0, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-6, -5), S(-3, -2), S(-1, 0), S(2, 3), S(5, 4), S(11, 1), S(6, -1), S(-13, -1)},
    {S(-5, -3), S(-1, -2), S(1, 0), S(2, 2), S(3, 2), S(7, 0), S(2, 0), S(-9, 0)},
    {S(-3, -2), S(-4, -3), S(-4, -2), S(-4, 1), S(0, 1), S(3, 1), S(5, 2), S(8, 1)},
    {S(-2, -11), S(0, -9), S(0, -4), S(-1, 2), S(-1, 6), S(3, 5), S(-1, 7), S(4, 4)},
...
const i32 pst_file[][8] = {
    ...
    {S(-2, 0), S(-2, 1), S(-1, 1), 0, S(1, -1), S(2, 0), S(2, 0), S(0, -1)},
    {S(-3, -5), S(-2, -3), S(-1, 0), S(0, 1), S(0, 3), S(1, 3), S(3, 0), S(2, 0)},
    {S(-2, -3), S(1, -1), S(-3, 1), S(-5, 2), S(-5, 2), S(-2, 1), S(1, 0), S(1, -3)},
};
const i32 open_files[][3] = {
    {S(28, 19), S(6, 16), S(-25, 7)},
    {S(58, 10), S(-6, 38), S(-69, -4)},
};
const i32 pawn_protection[] = {S(26, 13), S(5, 16), S(2, 7), S(10, 4), S(-9, 13), S(-35, 24)};
const i32 passers[] = {S(-9, 13), S(14, 43), S(43, 108), S(191, 188)};
const i32 pawn_passed_protected = S(15, 18);
const i32 pawn_doubled = S(-15, -32);
const i32 pawn_phalanx = S(12, 13);
const i32 pawn_passed_blocked[] = {S(-7, -19), S(9, -41), S(11, -78), S(21, -102)};
const i32 pawn_passed_king_distance[] = {S(2, -6), S(-4, 10)};
const i32 bishop_pair = S(33, 65);
const i32 king_shield[] = {S(42, -9), S(32, -9)};
