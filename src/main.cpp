...
const int max_material[] = {127, 411, 446, 769, 1466, 0, 0};
const int material[] = {S(96, 127), S(348, 411), S(362, 446), S(457, 769), S(926, 1466), 0};
const int pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(0, -1), S(2, -1), S(5, 2), 0, 0},
    {S(-5, -5), S(-3, -2), S(-1, 0), S(2, 3), S(5, 4), S(10, 1), S(5, -1), S(-13, -1)},
    {S(-4, -2), S(-1, -2), S(1, 0), S(2, 1), S(3, 2), S(6, 0), S(2, 0), S(-8, 0)},
    {S(-3, -2), S(-4, -3), S(-5, -2), S(-4, 0), S(0, 1), S(3, 1), S(5, 2), S(7, 2)},
    {S(-2, -7), S(0, -11), S(0, -6), S(-1, 2), S(-1, 6), S(3, 4), S(-2, 8), S(4, 4)},
    {S(0, -5), S(0, -1), S(-1, 0), S(-4, 2), S(-1, 3), S(6, 2), S(4, 1), S(3, -4)},
};
const int pst_file[][8] = {
    {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(2, 0), S(-2, 0)},
    {S(-5, -4), S(-2, -1), S(0, 2), S(2, 3), S(2, 3), S(2, 1), S(1, -1), S(-1, -3)},
    {S(-3, -2), 0, 0, S(0, 1), S(0, 1), S(0, 1), S(2, 0), S(0, -2)},
    {S(-1, 0), S(-2, 1), S(-1, 1), 0, S(1, -1), S(1, 0), S(2, -1), S(-1, -1)},
    {S(-3, -5), S(-1, -3), S(-1, -1), S(-1, 3), S(-1, 3), S(1, 3), S(3, 0), S(2, 0)},
    {S(-1, -3), S(2, -1), S(-3, 1), S(-5, 2), S(-5, 2), S(-2, 2), S(2, 0), S(2, -3)},
};
const int open_files[][3] = {
    {S(26, 18), S(5, 21), S(-23, 6)},
    {S(53, 12), S(-3, 31), S(-62, -5)},
};
const int pawn_protection[] = {S(24, 14), S(3, 17), S(2, 8), S(8, 6), S(-11, 18), S(-36, 24)};
const int passers[] = {S(-11, 17), S(11, 47), S(40, 110), S(181, 198)};
const int pawn_passed_protected = S(15, 17);
const int pawn_doubled = S(-14, -30);
const int pawn_phalanx = S(12, 14);
const int pawn_passed_blocked[] = {S(-8, -19), S(7, -42), S(12, -77), S(15, -90)};
const int pawn_passed_king_distance[] = {S(2, -6), S(-4, 9)};
const int bishop_pair = S(29, 66);
const int king_shield[] = {S(38, -8), S(28, -6)};
