...
const i32 max_material[] = {128, 412, 441, 767, 1467, 0, 0};
const i32 material[] = {S(101, 128), S(368, 412), S(385, 441), S(483, 767), S(960, 1467), 0};
const i32 pst_rank[][8] = {
    {0, S(-3, 0), S(-3, -1), S(0, -1), S(2, 0), S(5, 2), 0, 0},
    {S(-6, -5), S(-3, -2), S(-1, 0), S(1, 3), S(5, 4), S(10, 1), S(5, -1), S(-12, -1)},
    {S(-4, -3), S(-1, -2), S(1, 0), S(2, 2), S(3, 2), S(6, 1), S(2, 0), S(-8, 0)},
    {S(-2, -2), S(-4, -3), S(-4, -2), S(-4, 0), S(-1, 1), S(3, 1), S(5, 2), S(7, 1)},
    {S(-1, -11), S(0, -9), S(0, -4), S(-1, 2), S(-1, 6), S(2, 4), S(-2, 7), S(3, 4)},
    {S(0, -5), S(0, -2), S(-2, 0), S(-5, 2), S(-2, 3), S(6, 2), S(4, 1), S(2, -4)},
};
const i32 pst_file[][8] = {
    {S(-2, 0), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(3, 0), S(-2, 0)},
    {S(-5, -4), S(-2, -1), S(1, 2), S(2, 3), S(2, 3), S(3, 1), S(1, -1), S(-2, -4)},
    {S(-3, -2), 0, S(1, 0), S(1, 2), S(0, 2), S(0, 1), S(2, -1), S(-1, -2)},
    {S(-1, 0), S(-2, 1), S(-1, 1), 0, S(1, -1), S(2, 0), S(2, 0), S(0, -1)},
    {S(-3, -5), S(-2, -2), S(-1, 0), S(0, 1), S(0, 3), S(1, 3), S(3, 0), S(2, 0)},
    {S(-2, -3), S(1, -1), S(-3, 1), S(-5, 2), S(-5, 2), S(-2, 1), S(2, 0), S(1, -3)},
};
const i32 open_files[][5] = {
    {S(0, 2), S(-7, 16), S(27, 19), S(6, 17), S(-26, 8)},
    {S(-5, -12), S(-12, -2), S(57, 11), S(-7, 38), S(-69, -4)},
};
const i32 pawn_protection[] = {S(26, 13), S(5, 16), S(2, 6), S(11, 4), S(-9, 13), S(-36, 24)};
const i32 passers[] = {S(-7, 11), S(18, 41), S(51, 104), S(193, 185)};
const i32 pawn_passed_protected = S(16, 18);
const i32 pawn_doubled = S(-15, -34);
const i32 pawn_phalanx = S(12, 12);
const i32 pawn_passed_blocked[] = {S(-9, -13), S(9, -36), S(11, -74), S(23, -97)};
const i32 pawn_passed_king_distance[] = {S(2, -6), S(-4, 10)};
const i32 bishop_pair = S(34, 65);
