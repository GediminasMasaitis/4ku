...
const int max_material[] = {123, 397, 393, 722, 1304, 0, 0};
const int material[] = {S(97, 123), S(397, 386), S(393, 381), S(521, 722), S(1194, 1304), 0};
const int pst_rank[][8] = {
    {0, S(-6, 0), S(-5, -2), S(-1, -3), S(3, -1), S(8, 5), 0, 0},
    {S(-7, -7), S(-3, -3), S(2, -1), S(5, 6), S(8, 8), S(17, 1), S(9, -3), S(-30, -2)},
    {S(-3, 0), S(2, -3), S(2, -2), S(2, 0), S(4, 1), S(7, 0), S(0, 1), S(-15, 3)},
    {S(-1, -3), S(-6, -4), S(-6, -3), S(-6, 1), S(1, 2), S(5, 1), S(5, 4), S(7, 3)},
    {S(3, -10), S(4, -18), S(2, -9), S(-1, 4), S(-3, 11), S(0, 9), S(-6, 10), S(0, 4)},
    {S(-6, -7), S(-5, 0), S(-1, 1), S(-3, 4), S(2, 5), S(20, 3), S(13, 2), S(-5, -5)},
};
const int pst_file[][8] = {
    {S(-4, 1), S(-2, 1), S(-2, 0), S(2, -3), S(3, 0), S(4, 0), S(3, 1), S(-5, 0)},
    {S(-9, -8), S(-2, -2), S(1, 3), S(4, 6), S(4, 5), S(4, 2), S(1, -1), S(-4, -5)},
    {S(-4, -1), S(1, -1), S(1, 0), 0, 0, S(-1, 0), S(4, 2), S(0, -1)},
    {S(-2, 1), S(-3, 1), S(0, 1), S(2, 0), S(2, -1), S(3, 0), 0, S(-1, -1)},
    {S(-3, -7), S(-1, -5), S(0, -2), S(-1, 5), S(-1, 4), S(1, 2), S(5, 0), S(1, 2)},
    {S(-3, -6), S(5, -3), S(-3, 2), S(-10, 4), S(-8, 4), S(-4, 3), S(3, 0), S(3, -7)},
};
const int open_files[][3] = {
    {S(18, 12), S(1, 29), S(-20, 4)},
    {S(43, 1), S(-4, 27), S(-52, -7)},
};
const int mobilities[] = {S(0, 2), S(4, 7), S(2, 3), S(1, 1), 0};
const int king_zone_mobilities[] = {S(8, -2), S(13, 0), S(29, -8), S(20, 16), 0};
const int pawn_protection[] = {S(20, 17), S(1, 18), S(1, 22), S(4, 8), S(-11, 16), S(-39, 22)};
const int passers[] = {S(-18, 16), S(-4, 49), S(18, 116), S(154, 233)};
const int pawn_passed_protected = S(12, 16);
const int pawn_doubled = S(-17, -27);
const int pawn_phalanx = S(12, 16);
const int pawn_passed_blocked[] = {S(-10, -16), S(2, -35), S(-4, -71), S(17, -111)};
const int pawn_passed_king_distance[] = {S(3, -6), S(-3, 9)};
const int bishop_pair = S(23, 68);
const int king_shield[] = {S(31, -8), S(22, -7)};
