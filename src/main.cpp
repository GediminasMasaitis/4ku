...
const i32 max_material[] = {101, 452, 476, 631, 1424, 0, 0};
const i32 material[] = {S(100, 101), S(452, 322), S(476, 344), S(631, 577), S(1424, 997), 0};
const i32 pst_rank[][8] = {
    {0, S(-2, 1), S(-3, 0), S(0, -1), S(2, 0), S(3, 1), 0, 0},
    {S(-4, -4), S(-2, -2), 0, S(3, 2), S(4, 2), S(9, 0), S(3, 0), S(-12, 2)},
    {S(-4, -2), S(-1, -1), S(1, 0), S(2, 0), S(2, 1), S(6, 0), S(-1, 1), S(-5, 1)},
    {S(-2, -3), S(-4, -2), S(-3, -1), S(-3, 1), S(1, 1), S(2, 1), S(4, 2), S(5, 2)},
    {S(-2, -7), S(0, -5), S(-1, -2), 0, S(0, 4), S(2, 3), S(-1, 4), S(2, 2)},
    {S(1, -5), S(0, -2), S(-4, 0), S(-5, 1), S(-7, 2), S(8, 0), S(8, 0), S(10, -5)},
};
const i32 pst_file[][8] = {
    {S(-2, 0), S(-1, 0), 0, 0, 0, S(3, 0), S(2, 0), S(-3, 0)},
    {S(-5, -3), S(-1, -1), S(1, 2), S(2, 3), S(2, 2), S(2, 1), S(1, 0), S(-1, -3)},
    {S(-2, -1), 0, 0, S(0, 1), S(0, 1), S(0, 1), S(2, 0), S(0, -1)},
    {S(-2, 0), S(-1, 0), 0, S(1, 0), S(2, -1), S(1, 1), 0, S(-1, 0)},
    {S(-2, -1), S(-2, -1), S(0, 1), S(0, 2), S(1, 1), S(1, 1), S(2, -1), S(1, -1)},
    {S(3, -1), S(2, 0), S(-2, 1), S(-3, 2), S(-6, 2), S(-2, 1), S(1, 2), S(1, -1)},
};
const i32 open_files[][3] = {
    {S(22, 18), S(5, 12), S(-24, 15)},
    {S(41, 3), S(-8, 23), S(-61, -1)},
};
const i32 pawn_protection[] = {S(24, 9), S(1, 19), S(7, 15), S(-1, 16), S(3, 18), S(20, 6)};
const i32 passers[] = {S(14, 13), S(41, 35), S(116, 83), S(302, 119)};
const i32 pawn_passed_protected = S(16, 20);
const i32 pawn_doubled = S(-25, -27);
const i32 pawn_phalanx = S(13, 16);
const i32 pawn_passed_blocked[] = {S(-12, -18), S(5, -38), S(17, -58), S(-77, -15)};
const i32 pawn_passed_king_distance[] = {S(-1, -4), S(-1, 7)};
const i32 bishop_pair = S(31, 47);
const i32 king_shield[] = {S(36, -9), S(22, -6)};
