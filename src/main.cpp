...
const int max_material[] = {123, 395, 423, 736, 1459, 0, 0};
const int material[] = {S(123, 116), S(394, 395), S(411, 423), S(521, 736), S(934, 1459), 0};
const int pst_rank[][8] = {
    {0, S(-7, 0), S(-8, 0), S(-2, -2), S(3, -1), S(13, 2), 0, 0},
    {S(-15, -9), S(-10, -3), S(-3, 1), S(2, 6), S(10, 7), S(22, 2), S(13, -1), S(-19, -2)},
    {S(-11, -5), S(-2, -2), S(1, 0), S(3, 3), S(6, 4), S(14, 1), S(5, -1), S(-16, 0)},
    {S(-5, -4), S(-7, -6), S(-9, -4), S(-9, 1), S(-2, 3), S(6, 4), S(10, 5), S(15, 2)},
    {S(-5, -17), S(-1, -17), S(-1, -11), S(-4, 4), S(-2, 12), S(6, 8), S(-1, 14), S(8, 7)},
    {S(4, -11), S(0, -2), S(-7, 2), S(-15, 7), S(-7, 9), S(5, 10), S(4, 7), S(5, -4)},
};
const int pst_file[][8] = {
    {S(-5, 2), S(-3, 2), S(-3, 0), S(0, -2), S(2, -1), S(4, 0), S(7, 0), S(-3, -1)},
    {S(-13, -5), S(-5, -1), S(1, 3), S(5, 5), S(6, 5), S(6, 2), S(3, -2), S(-4, -7)},
    {S(-6, -3), S(-1, 0), S(1, 1), S(1, 2), S(2, 3), S(1, 2), S(5, -1), S(-2, -4)},
    {S(-4, 1), S(-5, 2), S(-3, 2), 0, S(3, -2), S(3, 0), S(6, -1), S(0, -2)},
    {S(-9, -7), S(-5, -2), S(-2, 1), S(0, 3), S(0, 5), S(3, 5), S(7, 0), S(6, -5)},
    {S(-5, -6), S(0, -1), S(-10, 3), S(-7, 3), S(8, 0), S(-4, 3), S(1, 1), S(-2, -5)},
};
const int open_files[][3] = {
    {S(31, 17), S(9, 15), S(-27, 10)},
    {S(64, 7), S(-10, 42), S(-83, 1)},
};
const int pawn_protection[] = {S(31, 12), S(6, 12), S(3, 6), S(11, 2), S(-10, 12), S(-32, 22)};
const int passers[] = {S(4, 11), S(38, 39), S(79, 96), S(279, 166)};
const int pawn_passed_protected = S(15, 19);
const int pawn_doubled = S(-19, -30);
const int pawn_phalanx = S(14, 11);
const int pawn_passed_blocked[] = {S(-12, -17), S(7, -42), S(11, -75), S(-16, -85)};
const int pawn_passed_king_distance[] = {S(1, -5), S(-4, 9)};
const int bishop_pair = S(39, 62);
const int king_shield[] = {S(46, -7), S(37, -8)};
