...
const int max_material[] = {124, 390, 427, 739, 1343, 0, 0};
const int material[] = {S(89, 124), S(372, 390), S(391, 427), S(494, 739), S(1118, 1343), 0};
const int pst_rank[][8] = {
    {0, S(-6, 1), S(-5, -1), S(-1, -3), S(4, -1), S(8, 5), 0, 0},
    {S(-9, -8), S(-4, -3), 0, S(4, 7), S(9, 8), S(18, 1), S(10, -3), S(-28, -3)},
    {S(-8, -4), S(-1, -4), S(1, -1), S(3, 3), S(5, 4), S(10, 1), S(2, 0), S(-13, 0)},
    {S(-5, -2), S(-9, -4), S(-8, -4), S(-7, 1), S(1, 2), S(6, 1), S(9, 3), S(14, 3)},
    {S(-3, -14), S(0, -20), S(0, -11), S(-2, 4), S(-1, 11), S(5, 8), S(-4, 12), S(4, 9)},
    {S(-5, -4), S(-5, 3), S(-3, 4), S(-5, 7), S(0, 8), S(20, 5), S(11, 4), S(-4, -2)},
};
const int pst_file[][8] = {
    {S(-3, 1), S(-2, 1), S(-2, 0), S(1, -3), S(2, 0), S(4, 0), S(4, 1), S(-4, 0)},
    {S(-8, -9), S(-2, -2), S(1, 3), S(3, 7), S(3, 6), S(4, 3), S(2, -1), S(-3, -6)},
    {S(-5, -4), S(0, -1), S(1, 1), S(0, 2), S(0, 3), S(0, 2), S(4, 1), S(0, -4)},
    {S(-2, 0), S(-3, 1), S(-1, 1), S(1, 0), S(1, 0), S(2, 0), S(2, 0), S(0, -2)},
    {S(-5, -10), S(-1, -8), S(0, -4), S(-1, 5), S(-2, 6), S(1, 5), S(6, 1), S(3, 4)},
    {S(-2, -3), S(4, 1), S(-5, 5), S(-12, 8), S(-10, 7), S(-5, 6), S(3, 4), S(4, -4)},
};
const int open_files[][3] = {
    {S(25, 17), S(2, 28), S(-19, 4)},
    {S(51, 12), S(-1, 31), S(-51, -8)},
};
const int pawn_protection[] = {S(20, 16), S(3, 20), S(0, 11), S(7, 8), S(-10, 16), S(-35, 22)};
const int passers[] = {S(-22, 17), S(-11, 49), S(9, 116), S(117, 225)};
const int pawn_passed_protected = S(14, 16);
const int pawn_doubled = S(-14, -27);
const int pawn_phalanx = S(10, 15);
const int pawn_passed_blocked[] = {S(-7, -16), S(8, -36), S(-1, -69), S(39, -112)};
const int pawn_passed_king_distance[] = {S(3, -6), S(-4, 9)};
const int bishop_pair = S(24, 67);
const int king_shield[] = {S(32, -7), S(24, -7)};
