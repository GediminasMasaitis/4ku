...
const int max_material[] = {115, 391, 410, 721, 1391, 0, 0};
const int material[] = {S(115, 114), S(391, 379), S(405, 410), S(503, 721), S(975, 1391), 0};
const int pst_rank[][8] = {
    {0, S(-3, 0), S(-4, 0), S(-1, -1), S(2, 0), S(6, 2), 0, 0},
    {S(-6, -4), S(-4, -1), S(-1, 0), S(2, 3), S(5, 3), S(11, 0), S(6, -1), S(-13, -1)},
    {S(-5, -2), S(-1, -1), S(1, 0), S(2, 1), S(3, 2), S(7, 0), S(2, 0), S(-9, 0)},
    {S(-2, -2), S(-4, -2), S(-4, -2), S(-4, 0), S(-1, 1), S(3, 1), S(5, 2), S(7, 1)},
    {S(-2, -8), S(-1, -8), S(0, -5), S(-2, 2), S(-2, 6), S(3, 3), S(-1, 6), S(4, 3)},
    {S(0, -5), S(0, -1), S(-3, 0), S(-7, 2), S(-2, 3), S(5, 2), S(3, 1), S(2, -3)},
};
const int pst_file[][8] = {
    {S(-2, 1), S(-1, 1), S(-1, 0), S(0, -1), S(1, 0), S(2, 0), S(3, 0), S(-2, 0)},
    {S(-6, -3), S(-2, 0), S(1, 2), S(2, 3), S(3, 2), S(3, 1), S(1, -1), S(-2, -4)},
    {S(-3, -1), 0, S(1, 0), S(1, 1), S(1, 2), S(0, 1), S(2, -1), S(-1, -2)},
    {S(-2, 0), S(-2, 1), S(-1, 1), 0, S(2, -1), S(2, 0), S(2, 0), S(-1, -1)},
    {S(-4, -4), S(-2, -2), S(-1, 0), S(0, 1), S(0, 3), S(1, 3), S(3, 0), S(2, -1)},
    {S(-1, -3), S(2, -1), S(-2, 1), S(-3, 1), S(-5, 2), S(0, 1), S(2, 0), S(1, -3)},
};
const int open_files[][3] = {
    {S(29, 18), S(6, 19), S(-25, 8)},
    {S(61, 7), S(-11, 43), S(-75, 0)},
};
const int pawn_protection[] = {S(27, 13), S(7, 12), S(3, 7), S(10, 3), S(-9, 11), S(-31, 21)};
const int passers[] = {S(0, 10), S(29, 38), S(56, 101), S(213, 184)};
const int pawn_passed_protected = S(15, 17);
const int pawn_doubled = S(-17, -28);
const int pawn_phalanx = S(10, 12);
const int pawn_passed_blocked[] = {S(-10, -16), S(6, -38), S(17, -78), S(16, -109)};
const int pawn_passed_king_distance[] = {S(1, -5), S(-4, 10)};
const int bishop_pair = S(38, 58);
const int king_shield[] = {S(46, -12), S(35, -11)};
