...
const int max_material[] = {122, 427, 447, 743, 1328, 0, 0};
const int material[] = {S(99, 122), S(427, 384), S(447, 422), S(561, 743), S(1289, 1328), 0};
const int pst_rank[][8] = {
    {0, S(-6, 1), S(-6, -2), S(0, -3), S(4, -1), S(8, 5), 0, 0},
    {S(-9, -7), S(-4, -2), 0, S(4, 7), S(9, 8), S(20, 1), S(8, -3), S(-29, -4)},
    {S(-8, -4), S(-1, -3), S(2, 0), S(4, 3), S(5, 4), S(10, 1), S(2, 0), S(-13, -1)},
    {S(-5, -2), S(-9, -4), S(-8, -3), S(-7, 1), S(0, 2), S(6, 1), S(10, 3), S(13, 3)},
    {S(-2, -16), S(0, -19), S(0, -10), S(-2, 5), S(-2, 12), S(5, 7), S(-3, 12), S(4, 8)},
    {S(-4, -7), S(-3, 1), S(-3, 2), S(-8, 5), S(-1, 6), S(22, 3), S(12, 2), S(2, -6)},
};
const int pst_file[][8] = {
    {S(-4, 1), S(-2, 1), S(-1, 0), S(2, -2), S(2, 0), S(4, 0), S(4, 1), S(-5, 0)},
    {S(-10, -8), S(-2, -2), S(1, 3), S(4, 7), S(4, 6), S(5, 2), S(2, -2), S(-4, -7)},
    {S(-5, -4), S(0, -1), S(1, 1), S(0, 3), S(0, 3), S(0, 2), S(5, 1), S(-1, -4)},
    {S(-2, 0), S(-3, 1), S(-2, 2), S(1, 0), S(2, -1), S(3, 0), S(1, 0), S(0, -2)},
    {S(-5, -11), S(-2, -7), S(0, -3), S(-1, 4), S(-1, 5), S(2, 5), S(5, 2), S(2, 4)},
    {S(0, -6), S(5, -2), S(-3, 2), S(-11, 4), S(-9, 4), S(-3, 3), S(4, 1), S(5, -6)},
};
const int open_files[][3] = {
    {S(29, 16), S(2, 29), S(-21, 5)},
    {S(60, 9), S(-3, 37), S(-56, -6)},
};
const int pawn_protection[] = {S(21, 18), S(5, 21), S(0, 12), S(8, 8), S(-10, 12), S(-36, 22)};
const int passers[] = {S(-25, 16), S(-13, 50), S(4, 121), S(135, 245)};
const int pawn_passed_protected = S(14, 15);
const int pawn_doubled = S(-17, -26);
const int pawn_phalanx = S(10, 17);
const int pawn_passed_blocked[] = {S(-4, -14), S(3, -32), S(2, -71), S(50, -127)};
const int pawn_passed_king_distance[] = {S(4, -6), S(-4, 10)};
const int bishop_pair = S(27, 68);
const int king_shield[] = {S(36, -10), S(27, -9)};
