...
const i32 max_material[] = {148, 520, 523, 952, 1781, 0, 0};
const i32 material[] = {S(89, 148), S(349, 520), S(362, 523), S(480, 952), S(1044, 1781), 0};
const i32 pst_rank[] = {
    0,         S(-3, 0),  S(-3, -1), S(-1, -1), S(1, 0),  S(5, 3),  0,        0,          // Pawn
    S(-2, -6), S(0, -3),  S(1, -1),  S(3, 3),   S(4, 4),  S(5, 1),  S(2, 0),  S(-15, 2),  // Knight
    S(0, -2),  S(2, -1),  S(2, 0),   S(2, 0),   S(2, 0),  S(2, 0),  S(-1, 1), S(-10, 2),  // Bishop
    S(0, -4),  S(-1, -4), S(-2, -2), S(-2, 1),  S(0, 2),  S(2, 2),  S(1, 3),  S(2, 1),    // Rook
    S(2, -11), S(3, -8),  S(2, -4),  S(0, 2),   S(0, 5),  S(-1, 5), S(-4, 7), S(-2, 4),   // Queen
...
const i32 pst_file[] = {
    ...
    S(-2, -5), S(2, -2),  S(-1, 1), S(-4, 2), S(-4, 2), S(-2, 2), S(2, -1), S(0, -5),   // King
};
...
const i32 mobilities[] = {S(8, 5), S(7, 6), S(3, 5), S(3, 2), S(-5, -1)};
const i32 king_attacks[] = {S(11, -5), S(18, -3), S(26, -9), S(18, 12), 0};
const i32 pawn_protection[] = {S(23, 16), S(2, 18), S(6, 18), S(9, 9), S(-8, 23), S(-29, 25)};
const i32 pawn_threat_penalty[] = {S(-4, 0), S(21, 0), S(12, 7), S(11, 19), S(9, 17), S(4, 9)};
const i32 passers[] = {S(16, 10), S(64, 31), S(122, 68), S(327, 154)};
const i32 pawn_passed_protected = S(11, 24);
const i32 pawn_doubled_penalty = S(11, 38);
const i32 pawn_phalanx = S(12, 16);
const i32 pawn_passed_blocked_penalty[] = {S(9, 16), S(4, 28), S(10, 42), S(86, 29)};
const i32 pawn_passed_free_to_advance[] = {S(-15, 15), S(-35, 45), S(-59, 116), S(-141, 175)};
const i32 pawn_passed_king_distance[] = {S(-1, -6), S(-2, 10)};
            ...
            while (copy) {
                ...
                if (p == Pawn) {
                    ...
                    if (rank > 2 && !(0x101010101010101ull << sq & (pawns[1] | attacked_by_pawns))) {
                        ...
                        // Passed pawns free to advance
                        else if (!is_attacked(pos, sq + 8))
                            score += pawn_passed_free_to_advance[rank - 3];
