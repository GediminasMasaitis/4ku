...
const int max_material[] = {122, 396, 423, 737, 1458, 0, 0};
const int material[] = {S(122, 117), S(394, 396), S(410, 423), S(519, 737), S(933, 1458), 0};
const int pst_rank[][8] = {
    {0, S(-7, 1), S(-8, 0), S(-2, -2), S(3, -1), S(13, 2), 0, 0},
    {S(-15, -9), S(-9, -3), S(-3, 1), S(2, 6), S(10, 7), S(22, 2), S(12, -1), S(-20, -2)},
    ...
    {S(2, -10), S(1, -2), S(-6, 2), S(-14, 7), S(-7, 9), S(4, 10), S(3, 7), S(5, -4)},
};
const int pst_file[][8] = {
    {S(-4, 2), S(-3, 2), S(-2, 0), S(0, -2), S(3, -1), S(4, 0), S(6, 0), S(-4, -1)},
    {S(-13, -5), S(-5, -1), S(1, 3), S(5, 5), S(6, 5), S(6, 2), S(3, -2), S(-4, -7)},
    {S(-6, -2), S(-1, 0), S(1, 1), S(1, 2), S(2, 3), S(1, 2), S(5, -1), S(-2, -4)},
    {S(-4, 1), S(-5, 2), S(-2, 2), S(1, 0), S(4, -2), S(4, 0), S(6, -1), S(-3, -1)},
    {S(-9, -7), S(-5, -2), S(-2, 2), S(0, 3), S(0, 5), S(3, 5), S(6, -1), S(6, -5)},
    {S(-3, -6), S(3, -2), S(-6, 2), S(-6, 3), S(-10, 4), S(-2, 2), S(4, 0), S(0, -5)},
};
const int open_files[][3] = {
    {S(31, 17), S(9, 15), S(-32, 11)},
    {S(64, 7), S(-10, 41), S(-84, 0)},
};
const int pawn_protection[] = {S(30, 13), S(7, 11), S(4, 5), S(11, 2), S(-10, 12), S(-31, 21)};
const int passers[] = {S(1, 10), S(39, 37), S(82, 94), S(269, 170)};
                ...
                if (p == Pawn) {
                    ...
                    if (rank > 2 && !(0x101010101010101ULL << sq & (pawns[1] | attacked_by_pawns))) {
                        ...
                        if (north(piece_bb) & pos.colour[1])
                            score += (rank - 3) * pawn_passed_blocked;
